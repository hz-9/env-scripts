...
# Test script - for verifying basic functionality of install-jq.sh script
...
test_info "Starting tests for install-jq.sh script"
...
assert_file_exists "$SCRIPT_PATH" "install-jq.sh script file exists"

# Checkpoint 2: Check if script is executable
assert_success "test -x '$SCRIPT_PATH'" "install-jq.sh script has execute permission"

# Checkpoint 3: Check script syntax
assert_success "bash -n '$SCRIPT_PATH'" "install-jq.sh script syntax is correct"
...
if [[ "$script_output" != "No help option" ]]; then
    assert_contains "$script_output" "jq" "Script output contains jq-related information"
fi
