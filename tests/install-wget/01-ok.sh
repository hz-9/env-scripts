...
# Test script - for verifying basic functionality of install-wget.sh script
...
test_info "Starting tests for install-wget.sh script"
...
assert_file_exists "$SCRIPT_PATH" "install-wget.sh script file exists"

# Checkpoint 2: Check if script is executable
assert_success "test -x '$SCRIPT_PATH'" "install-wget.sh script has execute permission"

# Checkpoint 3: Check script syntax
assert_success "bash -n '$SCRIPT_PATH'" "install-wget.sh script syntax is correct"
...
if [[ "$script_output" != "No help option" ]]; then
    assert_contains "$script_output" "wget" "Script output contains wget-related information"
fi
